...
let coveredWords = []
	let result = words.forEach(async (word)=>{
...
handleChangeRowsPerPage = event => {
	...
	handleAddToComplete = async word => {
		// const url = this.state.user.words.includes(word)?`${Config.API}/topics/${this.props.topicId}/unlike`:`${Config.API}/topics/${this.props.topicId}/like`;
		// const res = await request(options(url,'post')); //method cat id data
		// let user = this.state.user;
		// user.likes.includes(this.props.topicId)?user.likes.splice(user.likes.indexOf(this.props.topicId),1):user.likes.push(this.props.topicId)
		// this.setState({
		//   user
		// });
	};
	//const options = (url, method = 'get', data = null)
	...
	handleGetKnownWords = async () => {
		try {
			...
			const res = await request(options(`${Config.API}/words?top=5000`, 'get', data));
	...
	handleAddToKnown = async word => {
		...
		knownWords.indexOf(words[objIndex].english) === -1 && knownWords.push(words[objIndex].english)
	...
	<hr />
				...
				<Grid item xs={12}>
					<Paper style={{ minHeight:"90px",height: '100%', margin: '16px',padding:'4px' }}>
						<Grid
							container
