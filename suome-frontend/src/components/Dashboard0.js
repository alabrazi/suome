...
class Dashboard extends React.Component {
	...
	fetchTopics = async () => {
		try {
			...
			const url50 = `${Config.API}/topics?page=${this.page}&per_page=${50}`;
	...
	handleAddToLike = async (topicID)=>{
		// if(this.state.user.completed.includes(topicID)){
			...
			const url = this.state.user.likes.includes(topicID)?`${Config.API}/topics/${topicID}/unlike`:`${Config.API}/topics/${topicID}/like`;
...
topics[docIndex].likes=topics[docIndex].likes-1
	...
	handleAddToCompleted = async (topicID)=>{
				if(this.state.user.completed.includes(topicID))
				{
					alert("Topic is marked as completed");
					// let topics =this.state.topics
					// topics[topicID].likes =topics[topicID].likes -1
					// this.setState({topics})
				}else{
					alert("Topic is not yet completed");
					// let topics =this.state.topics
					// topics[topicID].likes =topics[topicID].likes +1
					// this.setState({topics})
				}
	...
	buildInfo = async (words, tags, description, id,title,type,action) => {
		let stopwords = [  "but", "there", "have", "with", "they",  "an", "be",  "for", "do", "it", "your", "of", "most", "other",  "is", "am", "or", "who", "as",  "him", "the", "themselves", "are", "we", "these", "your", "his",   "me", "were", "her",  "himself", "this", "should", "our", "their", "to", "our", "had", "she",  "no", "when", "at", "any", "before", "them",  "and", "been", "have", "in", "will", "on", "does", "yourselves", "then", "that", "what", "why", "so", "can", "did", "not", "he", "you", "herself", "has", "where", "myself", "which", "those", "i",   "whom", "being", "if", "their", "my", "a", "by", "doing", "it", "how",  "was", "here"]
...
win.focus();
		...
		return (
			<div className={classes.root}>
				<Header user={this.state.user} isReadme ={true} />
					{this.page > 1 && <Grid container justify="center">
										<Grid item>
											<Button style={{marginTop:"16px"}} onClick={()=>{this.loadMore('previous')}}>Load Previous</Button>
										</Grid>
									</Grid>}
				<Paper className={classes.paper}>
					...
					<GridList cellHeight={400} cols={2} spacing={16} className={classes.gridList}>
						...
						{this.state.loading === "false" ? this.state.topics.map(topic => (
	...
	// }
								/>
								{
									 JSON.parse(localStorage.getItem('user')) && <GridListTileBar style={{marginTop:"49px"}}
										titlePosition="top"
										key={topic._id+"top2"}
										title={
											this.state.user
												? [
													...
													<IconButton
														className={classes.icon}
														onClick={() => {
															this.buildInfo(topic.featuredWords, topic.tags, topic.description, topic._id,topic.title,'info',true,this.state.newWords);
													...
													<IconButton  className={classes.icon}
													 ...
													 <Icon className={classNames(classes.icon,this.state.user && this.state.user.completed  && this.state.user.completed.includes(topic._id)?"fas fa-check-circle":"far fa-check-circle")}
														 onClick={(e)=>{this.handleAddToCompleted(topic._id)}}/>
												 </IconButton>
													</Grid>
													<Grid item>
												...
												<Badge badgeContent={topic.likes} color="primary" classes={{ badge: classes.badge }}>
													 <Icon className={classNames(classes.icon,this.state.user && this.state.user.likes && this.state.user.likes.includes(topic._id)?"fas fa-heart":"far fa-heart")}
														 />
												</Badge>
	...
	this.buildInfo(topic.featuredWords, topic.tags, topic.description, topic._id,topic.title,'info',true,this.state.newWords);
								...
								<GridListTileBar style={{height:"6rem"}}
									...
									<Button className={classes.button} disableFocusRipple
									          ...
									          ></Chip>
													...
													{
														this.state.newWords[topic._id] && <Grid key={topic._id+"newWords"} item>
				...
				{this.state.loading !== "true" && this.state.topics.length > 1  && 				<Grid container justify="center">
									<Grid item>
										<Button onClick={()=>{this.loadMore('more')}}>Load More</Button>
									</Grid>
								</Grid>}
